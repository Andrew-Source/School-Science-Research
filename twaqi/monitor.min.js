...
function writemenu(res) {
    res.write(`
        ...
        <ul>
            <li style="width:100vw;"><div style="margin: auto;"><a href="/info">Taiwan AQI data monitor developed by <b><i>Andrew</i> @ Munucial</b></a></div></li>
        </ul>
    `);
}
...
cron.schedule("1 0 * * * *", function() {
    ...
    if (s.search) {
        ...
        if (0 != u.length) {
            for (e.write("<table id=\"datatable\">"), e.write("<tr><th>地點</th><th>AQI</th><th>重點汙染值</th></tr>"), i = 0; i < u.length; i++) e.write("<tr><td>" + u[i].name + "</td><td>" + u[i].aqi + "</td><td>" + u[i].pollutant.key + ": " + u[i].pollutant.value + "</td></tr>");
            e.write("</table>");
        } else {
            e.write("<h1>No result founded, we start recording since 2017/11/18 3:00 UTC+8</h1>");
        }
    e.end('</body></html>');
});
...
app.get("/info", function(t, e) {
    var totalrecored;
    if(jsonfile.readFileSync('./data/data.json').基隆) {
        totalrecored = Object.keys(jsonfile.readFileSync('./data/data.json').基隆).length
    } else {totalrecored = 0;};
    e.status(200).send("<h1>App info:</h1><h2>Started at: "+starttime+"</h2><h2>Data catch frequency: 1hr</h2><h2>Data archiving since 2017/11/18 3:00 UTC+8</h2><h2>Data archived (since app start): "+datarecorded+"</h2><h2>Data archived (total): "+totalrecored);
});
